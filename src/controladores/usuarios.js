...
const cadastrarUsuario = async (req, res) => {
    ...
    try {
        const dados = await knex('usuarios').where('email', email);
        ...
        const usuario = await knex('usuarios').insert({ nome: nome, email: email, senha: senhaCriptografada, nome_loja: nome_loja }).returning('*');
...
const atualizarPerfil = async (req, res) => {
    ...
    try {
        ...
        if (senha) {
            senha2 = await bcrypt.hash(senha, 10);
        }

        const atualizacaoUsuario = await knex('usuarios').update({ nome, email, senha: senha2, nome_loja }).where({ id })
